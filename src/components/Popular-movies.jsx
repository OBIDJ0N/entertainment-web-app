...
import { getPopularStart, getPopularSuccess, getPopularFailure } from "../slice/content";
import SmallCards from "./Small-cards";

const PopularMovies = () => {
    const { popularMovies, isLoading } = useSelector(state => state.content);
    useFetchContent(ContentService.getContent, getPopularStart, getPopularSuccess, getPopularFailure, 'movie', 'popular')

    return (
        <>
            <Typography className='text-white text-3xl max-phone:text-[1.25rem] mb-8 mt-10'>
                Popular  <Button variant="outlined" disableTouchRipple className="border-red text-red cursor-auto hover:bg-transparent max-tablet:px-4 max-tablet:text-sm">Movies</Button>
            </Typography>
            {isLoading ? (
                ...
                <Stack className="grid grid-cols-5 gap-x-10 gap-y-8 max-desktop:grid-cols-4 max-laptop:grid-cols-3 mr-[2.25rem] max-tablet:mr-0 max-phone:grid-cols-2 max-tablet:gap-x-[1.875rem] max-tablet:gap-y-6 max-phone:gap-4">
                    {popularMovies.results && popularMovies.results.slice(0, 15).map(item => (
